...
class trendmovieadapter(private val movielist: List<Movie_listItem>):
    RecyclerView.Adapter<trendmovieadapter.ViewHolder>() {


    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val inflate: View = LayoutInflater.from(parent.context).inflate(R.layout.movie_holder,parent,false)
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if (movielist[position].rating < 4) {
            holder.itemView.visibility = View.GONE
            holder.itemView.layoutParams = RecyclerView.LayoutParams(0, 0)
        }
        if (movielist[position].rating > 4){
            //Log.d("trend-movie-id","${movielist[position].id}")
        }
       // Log.d("Response", "movies : ${movielist.size}")

       //Log.d("from-actvity","$cover")
    ...
    inner  class ViewHolder(itemView: View): RecyclerView.ViewHolder(itemView) {
        ...
        var cov = itemView.findViewById<ImageView>(R.id.cov)
        var title = itemView.findViewById<TextView>(R.id.mov_title)
        var ratingBar = itemView.findViewById<RatingBar>(R.id.ratingBar1)
        var no_rat =itemView.findViewById<TextView>(R.id.no_rat)
        fun bind(movies: Movie_listItem) {
            holder.id = movies.id
            Picasso.get().load(movies.COVER).into(cov)
